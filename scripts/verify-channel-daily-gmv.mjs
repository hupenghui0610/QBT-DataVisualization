/**
 * 验证：渠道映射表 A 列 = 指定渠道名（默认「测试」）时，列出 E 列所有达人 ID，
 * 并在订单表中仅汇总这些达人的按日 GMV（与 feishu-channel-order-trend 列规则一致）。
 * 用法：FEISHU_APP_ID=xxx FEISHU_APP_SECRET=xxx node scripts/verify-channel-daily-gmv.mjs
 * 可选：VERIFY_CHANNEL_NAME=测试
 */
const CHANNEL_NAME = (process.env.VERIFY_CHANNEL_NAME || '测试').trim();
...
async function main() {
  ...
  for (let ri = SKIP_ROWS; ri < ordValues.length; ri++) {
    ...
    if (!darenIds.has(daren)) continue;
  ...
  console.log('=== 渠道「' + CHANNEL_NAME + '」达人日 GMV（仅该渠道 E 列达人）===');
  ...
  console.log('--- 渠道表 A 列=' + CHANNEL_NAME + ' 的行（E 列达人 ID）---');
  console.log('行数', rowsByChannel.length);
  if (rowsByChannel.length === 0) {
    console.log('未找到该渠道名：请核对 VERIFY_CHANNEL_NAME 或表中 A 列是否完全一致（含空格）。');
  } else {
    console.log('达人 ID 列表（去重 ' + darenIds.size + '）:', [...darenIds].sort().join(', '));
  }
  console.log('--- 订单汇总 ---');
  console.log('计入行', rowsOpen, '已关闭跳过', rowsClosed, '无有效日期', rowsNoDay);
  console.log('--- 按日 GMV ---');
  if (dates.length === 0) {
    console.log('无数据：检查达人是否在订单 AO 列出现、或扩大 ORDER_RANGE');
  } else {
    let sum = 0;
